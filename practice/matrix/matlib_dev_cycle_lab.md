# Лабораторная работа: от MatLib notebook к научной Python-библиотеке

## Краткая идея

В этой лабораторной работе требуется превратить учебный код из ["LU разложение, блочные и ленточные матрицы"](practice/matrix/MatLib.ipynb) в небольшой устанавливаемый Python-пакет для работы с матрицами специальной структуры. Основной предмет работы — полный цикл разработки научного программного обеспечения: организация репозитория, командная работа через git, управление зависимостями, тестирование, оформление документации, сборка пакета, публикация на TestPyPI и использование опубликованного пакета в отдельном проекте.

Исходная точка: [файл с ноутбуком](practice/matrix/MatLib.ipynb). Его можно использовать как источник идей, алгоритмов и тестовых примеров. Итоговая работа не должна оставаться набором ячеек notebook: результатом должен быть пакет, который можно установить, импортировать, протестировать, собрать и использовать как внешнюю зависимость.
...
- писать базовые unit- и integration-тесты с `pytest`;
- поддерживать единый стиль кода с помощью `ruff`;
- оформлять пользовательскую и developer-документацию в Markdown через MkDocs;
- собирать wheel/source distribution и публиковать пакет на TestPyPI;
...
однако каждый участник реализуется свой функционал.

Задания разделены на обязательную и факультативную часть.
Необязательные задания позволяют освоить подходы к разработки
и инструменты, которые используются в современной разработке, 
...
- есть как минимум три матричных компонента: плотная матрица, блочная матрица и хотя бы один специальный тип матрицы, например ленточная или симметричная;
...
Имена директорий и модулей можно менять, но итоговая структура должна отделять код библиотеки, тесты, документацию и примеры.
...
Если `uv init` не создал git-репозиторий автоматически, выполните:
...
| D | `solvers.py`, численный пример, integration-тест |
...
После завершения части работы ветка должна быть объединена с `main`. Если используется GitHub, рекомендуется делать это через Pull Request. Если Pull Request не используется, merge должен быть виден в истории git.

Минимальное требование: каждый участник должен иметь хотя бы один содержательный commit, попавший в итоговую ветку `main`.

Issues в этой работе не обязательны. Их можно использовать как факультативное расширение.
...
Важно: NumPy можно использовать как численную опору и как reference implementation в тестах. Однако все классы библиотеки не должны быть пустыми обертками, которые во всех операциях сразу сводятся к плотной матрице. Хотя бы один матричный тип или одна операция должны использовать специальную структуру данных содержательно.
...
### Задание 4.4. Напишите integration-тест

Integration-тест должен проверять не отдельный метод, а работу нескольких частей библиотеки вместе.
...
Избегайте случайных матриц в обязательных тестах. Если случайность все же используется, фиксируйте seed.
...
## Этап 5. Ruff и локальный quality gate
...
Это локальный quality gate. Он заменяет обязательный CI в данной лабораторной работе.
...
- где смотреть tutorial и API.

### Задание 6.3. Напишите `docs/tutorial.md`

Tutorial должен показывать воспроизводимый сценарий использования библиотеки.
...
Этот файл предназначен для будущего разработчика проекта. Он должен содержать команды:
...
Токен нужно создать в TestPyPI и передать через переменную окружения. Не записывайте токен в репозиторий, README, notebook, shell history с публичным доступом или сообщения Pull Request.
...
### Ошибка 1. Весь код остается в notebook

Notebook можно оставить как черновик или пример, но библиотечный код должен находиться в `src/<package_name>/`.
...
Package index обычно не позволяет перезаписать уже опубликованный файл той же версии. Для исправлений увеличивайте версию, например с `0.1.0` до `0.1.1`.

## Факультативные задания

Факультативные задания не обязательны. Их цель — показать, как этот проект может быть приближен к реальной практике разработки научного софта.
...
Свяжите Pull Request с issue так, чтобы issue закрывался после merge.
...
Для этой лабораторной type checking является факультативным, потому что типизация рекурсивных блочных структур может оказаться сложнее основной темы работы.
...
CI не должен заменять локальную проверку. Он должен автоматизировать уже понятный quality gate.

### F6. Pre-commit hooks

Настройте pre-commit hooks для автоматического запуска `ruff` перед commit.
...
- [ ] Документация содержит tutorial, API и development-инструкции.
